...
SUPPORT_ROLE = "Ticket supporter"  # این رول دسترسی به تیکت خواهد داشت
...
async def create_ticket(interaction, reason):
    ...
    embed.set_footer(text="Blizzard Ticket System")
...
# ================= /ROB COMMAND =================
@bot.tree.command(name="rob", description="Send any text as green title")
@app_commands.describe(text="متنی که میخوای به عنوان Title ارسال بشه")
async def rob(interaction: discord.Interaction, text: str):
    embed = discord.Embed(
        title=text,  # هرچی کاربر نوشت تایتل میشه
        color=discord.Color.green()
    )
    embed.set_author(name="BLIZZARD", icon_url=bot.user.display_avatar.url)
    embed.set_footer(text=f"Requested by {interaction.user}")
    await interaction.response.send_message(embed=embed)

# ================= /LOSE COMMAND =================
@bot.tree.command(name="lose", description="Send any text as red title")
@app_commands.describe(text="متنی که میخوای به عنوان Title ارسال بشه")
